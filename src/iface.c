...
static double** OldIfaceValues;        // interface flows, WQ & temperature at previous time
static double** NewIfaceValues;        // interface flows, WQ & temperature at next time
...
/* START modification by Alejandro Figueroa | EAWAG */
//  iface_getIfaceTemp       (called by addIfaceInflows in routing.c)
/* END modification by Alejandro Figueroa | EAWAG */
...
//
{
    ...
    // --- create matrices for old & new interface flows, WQ and Temperature values
    OldIfaceValues = project_createMatrix(NumIfaceNodes,
                                         1+NumIfacePolluts+TempModel.active);
    ...
    // --- read in new interface flows , WQ and Temperature values
    readNewIfaceValues();
...
//
{
    ...
    NumIfacePolluts = NumIfacePolluts - 1 - TempModel.active;
    ...
    if (TempModel.active == 1)
    {
        //IfaceTemperature = (int*)calloc(1, sizeof(int));
        //if (!IfaceTemperature) return ERR_MEMORY;
        IfaceTemperature = -1;
    }
