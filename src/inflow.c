...
//     nodeID  temperatureID  tSeriesID  (TEMPERATURE  unitsFactor  scaleFactor  baseline  basePat)
//
{
    int    j;                          // object index
    int    param;                      // FLOW (-1), TEMPERATURE(-10) or pollutant index
    int    type = CONCEN_INFLOW;       // FLOW, CONCEN, MASS or TEMPERATURE inflow
    ...
    if ( param < 0 )
    {
        ...
        else if ( match(tok[1], w_FLOW) ) param = -1;
...
//  Format of data line is:
    /* START modification by Alejandro Figueroa | EAWAG */
//    nodeID  FLOW/pollutID/TEMPERATURE  avgValue  (pattern1 pattern2  ... pattern4)
//
{
    int    i;
    int    j;                          // node index
    /* START modification by Alejandro Figueroa | EAWAG */
    int    k;                          // pollutant index (-1 for flow, -10 for temperature)
